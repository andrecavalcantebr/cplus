...
static void emit_class(const mpc_ast_t *class_decl)
{
    ...
    char buf[8192] = {0};
    /* recursive appender */
    typedef struct { char *b; size_t n; size_t cap; } SB;
    void append(const char *s, SB *sb){ if(!s) return; size_t m=strlen(s); if (sb->n + m >= sb->cap) m = (sb->cap? sb->cap-1 - sb->n:0); if (m>0){ memcpy(sb->b+sb->n,s,m); sb->n+=m; sb->b[sb->n]=0; } }
    void append_rec(const mpc_ast_t *node, SB *sb){ if(!node) return; if(node->children_num==0){ if(node->contents && *node->contents) append(node->contents,sb); return; } for(int k=0;k<node->children_num;k++) append_rec(node->children[k],sb);} 
    ...
    if (!lb || !rb || !endsemi || endsemi < rb) return;
    ...
    while (p < body + blen)
    {
        ...
        {
            int is_method = 0;
            for (size_t i = 0; i < len; i++) { if (p[i] == '(') { is_method = 1; break; } }
            if (!is_method)
            {
                ...
                while (q < p + len && (*q == ' ' || *q == '\t' || *q == '\n' || *q == '\r')) q++;
                const char *after = q;
                if ((size_t)(p+len - q) >= 6 && strncmp(q, "public", 6) == 0) { after = q + 6; while (after < p+len && (*after==' '||*after=='\t')) after++; }
                else if ((size_t)(p+len - q) >= 7 && strncmp(q, "private", 7) == 0) { after = q + 7; while (after < p+len && (*after==' '||*after=='\t')) after++; }
                fputs("    ", stdout);
    ...
    while (p < body + blen)
    {
        ...
        if (len)
        {
            ...
            if (lpar)
            {
                ...
                while (name_start > p && ((name_start[-1] == '_') || (name_start[-1] >= '0' && name_start[-1] <= '9') || (name_start[-1] >= 'A' && name_start[-1] <= 'Z') || (name_start[-1] >= 'a' && name_start[-1] <= 'z'))) name_start--;
                /* If there's a leading access modifier before the return type, skip it in the return part */
                const char *ret_start = p;
                while (ret_start < name_start && (*ret_start==' '||*ret_start=='\t'||*ret_start=='\n'||*ret_start=='\r')) ret_start++;
                if ((size_t)(name_start - ret_start) >= 6 && strncmp(ret_start, "public", 6) == 0) { ret_start += 6; while (ret_start < name_start && (*ret_start==' '||*ret_start=='\t')) ret_start++; }
                else if ((size_t)(name_start - ret_start) >= 7 && strncmp(ret_start, "private", 7) == 0) { ret_start += 7; while (ret_start < name_start && (*ret_start==' '||*ret_start=='\t')) ret_start++; }
                /* print return part */
