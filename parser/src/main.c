...
static char *grab_typespec(const mpc_ast_t *ts)
{
    ...
    for (int i = 0; i < ts->children_num; i++)
    {
        ...
        if (strstr(c->tag, "ident") || strstr(c->tag, "ptr") || strstr(c->tag, "qual") || strstr(c->tag, "lbrack") || strstr(c->tag, "rbrack"))
        {
...
Module parse_cplus(const char *input)
{
    ...
    if (mpc_parse("<stdin>", input, TU, &r))
    {
        ...
        {
            if (strstr(ast->children[i]->tag, "iface"))
                nif++;
            if (strstr(ast->children[i]->tag, "classdecl"))
                ncl++;
        }
        M.ifaces = (Interface *)calloc(nif, sizeof *M.ifaces);
        M.nifaces = 0;
        M.classes = (Class *)calloc(ncl, sizeof *M.classes);
        M.nclasses = 0;
        ...
        for (int i = 0; i < ast->children_num; i++)
        {
            ...
            if (strstr(n->tag, "iface"))
            {
                ...
                for (int k = 0; k < n->children_num; k++)
                    if (strstr(n->children[k]->tag, "methoddecl"))
                        nm++;
                    ...
                    if (strstr(n->children[k]->tag, "methoddecl"))
                    {
                        ...
                        if (md->children_num > 3 && strstr(md->children[3]->tag, "paramlist"))
                        {
                            ...
                            for (int t = 0; t < pl->children_num; t++)
                                if (strstr(pl->children[t]->tag, "param"))
                                    np++;
                ...
                for (int k = 0; k < n->children_num; k++)
                {
                    ...
                    if (c->contents && strcmp(c->contents, "implements") == 0)
                    {
                        ...
                        while (t < n->children_num && strstr(n->children[t]->tag, "ident"))
                        {
                            cnt++;
                            t++;
                            if (t < n->children_num && strcmp(n->children[t]->contents, ",") == 0)
                                t++;
                            else
                                break;
                        }
                        cls->ifaces = (char **)calloc(cnt, sizeof *cls->ifaces);
                        cls->nifaces = 0;
                        t = k + 1;
                        while (t < n->children_num && strstr(n->children[t]->tag, "ident"))
                        {
